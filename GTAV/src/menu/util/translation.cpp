...
namespace base {
	...
	void translation::push(std::string string) {
		...
		using jsonf = nlohmann::json;
		static jsonf save;
		auto cheat_path = std::filesystem::path(Util::GetDocumentsPath()).append("aether").append("translation.table");
	...
	void translation::load() {
		auto config_path = std::filesystem::path(Util::GetDocumentsPath()).append("aether").append("translation.table");
